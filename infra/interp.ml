(* Library to package the BT interpreter - version with reset support *)

open Btrsem

module Btree = BT_gen_rsem(Skills)
...
(* C function implementing the reset action for a skill *)
external reset: string -> bool = "reset"

(* Equivalent term of type skills_reset *)
let reset_f s =
  reset (camlstring_of_coqstring (Skills.skillName s))

(* The function we shall actually export *)
let tick1 bt =
  Btree.tick bt input_f reset_f
