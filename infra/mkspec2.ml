...
(* This function will be used to map the arbitrary-branching BT read by
   load_bt to an equivalent binary-branching BT. *)
...
let _ =
  ...
  if !filename = "" then
    ...
    try
      ...
      let smv_spec =
        if !standalone then
          translate_spec (Btbin.make_spec btb)
        else
          translate_spec (Btbin.make_spec_ocra btb)
      in
    ...
    | Sys_error s -> Printf.eprintf "System error: %s\n" s;
      exit 2
    | Not_found -> Printf.eprintf "The supplied BT has a non-binary node, aborting\n";
      exit 1
